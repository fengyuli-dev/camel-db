...
let create
  ...
  try create_table db table_name (List.combine cols data_types)
  with IllegalName ->
    print_endline "The table name is duplicated. Please reenter.";
    db
...
let select (db : database) table_name cols filter_function =
  ...
  with TableDNE ->
    print_endline
      "The table in the select attempt is not in the current database."
