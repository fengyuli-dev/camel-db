...
let delete_row
  ...
  try
    ...
    let new_table = filter_table_rows old_table negated in
    {
      db with
      tables =
        (let key =
...
let insert_default_in_every_column (old_table : table) =
  get_new_table (old_table : table) insert_default_into_column (old_table.num_rows)
    ...
    let new_table =
      (let key =
       ...
       update_one_row_only table_with_default_inserted
         fieldname_type_value_list new_row_index) in let final_table = {new_table with num_rows = new_table.num_rows + 1} 
    in
    {
      db with
      tables =
